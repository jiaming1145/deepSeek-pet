...
// ------------------------------------------------------------------ actions
const A = {};
const poseReset = () => { for (const b of boneList) b.userData.pose = 0; S_.rootY = 0; S_.rootRot = 0; S_.rootX = 0; S_.rootScale = 1; S_.eyesClosed = 0; S_.mouthOpen = 0; S_.irisOff = [0, 0]; S_.speed = 0; };
...
A.sleep = (tau) => {
  ...
  S_.rootY = 0.0 + 0.16 * u;
  S_.rootX = S_.facing * 0.82 * u;                              // pivot is at the feet inside the mirrored group: slide so the lying body stays in frame
  S_.rootScale = 1 - 0.08 * u;                                   // and shrink a touch: lying, she is nearly as long as the window is wide
  S_.eyesClosed = u;
...
A.wake = (tau) => { const u = 1 - smooth(clamp(tau / 1.0, 0, 1)); A.sleep(1.0); S_.rootRot *= u; S_.rootY *= u; S_.rootX *= u; S_.rootScale = 1 - 0.08 * u; S_.eyesClosed = u; for (const b of boneList) b.userData.pose *= u; if (u <= 0) A.idle(tau); };
