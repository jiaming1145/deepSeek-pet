...
|---|---|---|---|
| eval, overall verdict over 46 prompts × 3 runs = 138 turns | **NOT MEASURED** | every one of 27 gates | [`eval-report-final.md`](eval-report-final.md) |
...
| cold-profile first message, virgin `--user-data-dir` | **382 ms (light) · 720 ms (dark)** in the committed `e2e-report.json` (fix round 2); the earlier 391 · 456 · 462 · 476 ms (round 1) and 688–721 ms (fix round 1) came from runs whose report was overwritten | ≤ 3 s | [`app-first-message-cold.png`](app-first-message-cold.png), `e2e-report.json` |
...
**What the dark captures prove (final-review M-18).** Every `app-*-dark.png` is taken with
Playwright's `page.emulateMedia({ colorScheme: 'dark' })`, not with Windows in dark mode: the main
process never touches `nativeTheme`, and Task 10 recorded that setting `nativeTheme.themeSource =
'dark'` did not flip the renderer's `prefers-color-scheme`. The shots therefore prove that the
stylesheet's dark palette renders correctly in the built app; they do **not** prove that the app
follows the OS theme. `nativeTheme` handling is carried to Phase 4 with the settings surface (theme
toggle) — see `deferred.md`.
...
| `task-3-static-system.txt` | T3 → regenerated by T10 | the exact bytes of the character-mode system block — the cached prefix; byte-identical to T3's. The committed blob is 4502 bytes, LF-only, and `Buffer.equals` `renderStaticSystem(card, motionKeys)` (re-verified in the final-review fix wave). If your working copy shows CRLF, that is your clone's `core.autocrlf` — compare against `git show HEAD:docs/evidence/phase2/task-3-static-system.txt`, which `.gitattributes` (`docs/evidence/** -text`) keeps byte-exact |
...
| `eval-report.json`, `eval-report.md` | T9 → regenerated in the final-review fix wave | the harness, the fixture and the **27** gate rows (17 axes + 10 shape, after I-12 / I-13 / M-20 added `in_character_flips`, `emojiMultiCount` and `markdownLintCount`) — over the **`--dry` recorded corpus**, not live output. Produced by `node run.mjs --dry` from the fix worktree; `config.fixture` / `config.character` were normalised to `D:\ds\…` per A-5 and the SUBSTITUTED banner re-applied; `config.dry` is still `true` |
...
| `app-first-message-cold.png` | T10 | the cold-profile first message on the desktop: virgin user-data dir, band up in 382 ms (light) / 720 ms (dark) per `e2e-report.json`, over her lower third |
...
| `e2e-report.json` | T10 | the Playwright run record for the Electron lane: 4 expected, 0 unexpected, 1 skipped — the skip is the `20 real turns` test, gated on `DEEPSEEK_API_KEY`. **Path-normalised per A-5, otherwise the run record:** the lane ran in a worktree, so 16bf16a rewrote the plain-text worktree paths to `D:\ds\…` and the final-review fix wave (M-22) did the same for the three base64 `stdout[].buffer` entries (decoded → replaced → re-encoded; every buffer re-decoded afterwards, 0 hits). Timings, stats and the console lines are untouched |
